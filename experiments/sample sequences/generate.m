...
imshow(padarray(orig,[256 256],255));
...
for i=1:10
    ...
    imshow(padarray(s,[256-round(dr/2) 256-round(dc/2)],255));
    frame=getframe;
    writeVideo(writerObj,frame);
...
for i=1:10
    ...
    imshow(padarray(r,[256-round(dr/2) 256-round(dc/2)],255));
    frame=getframe;
    writeVideo(writerObj,frame);
...
writeVideo(writerObj,frame);
for i=1:10
    t=padarray(orig,[256-5*i 256-5*i],255,'pre');
    t=padarray(t,[256+5*i 256+5*i],255,'post');
    imshow(t);
    frame=getframe;
    writeVideo(writerObj,frame);
end
...
for i=1:10
    ...
    scale=padarray(scale,[256-round(dr/2) 256-round(dc/2)],255);
    imshow(scale);
    frame=getframe;
    writeVideo(writerObj,frame);
end
...
for i=1:10
    ...
    frac=padarray(frac,[256-round(dr/2) 256-round(dc/2)],255);
    imshow(frac);
    frame=getframe;
    writeVideo(writerObj,frame);
end
